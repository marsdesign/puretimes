...
$j(document).ready(function() {
	...
	$j("div.fixed-header-spacer").addClass("new-fixed-header-spacer").removeClass("fixed-header-spacer");
	function inidNewFixHeaderSpacer(){
		var header_height = $j("#header").outerHeight(true);
		if($j(".category-nav").length > 0 && $j("body").hasClass("catalog-category-view") && $j(window).width() > 770){
			header_height += $j(".category-nav").outerHeight(true);
		}
		if($j(window).width() > 770){
			$j("div.new-fixed-header-spacer").height(header_height);
		}else{
			$j("div.new-fixed-header-spacer").height(header_height);
		}
	}
	inidNewFixHeaderSpacer();
	...
	$j("#contact-button-trigger").appendTo($j("#contactForm li:last-child"));
	$j("i.close").click(function(e){
	  	e.preventDefault();
  		$j.fancybox.close();
	});
	if($j("#mini-login-container").length > 0){
		$j("[href='/customer/account']").after($j("#mini-login-container"));
		$j("[href='/customer/account']").click(function(e){
			if($j(this).parents(".home-header").length > 0){
				return true;
			}else{
				e.preventDefault();
				$j("#mini-login-container").slideToggle();
			}
		});
	}else{
		$j("[href='/customer/account']").parent().after('<li><a href="/customer/account/logout">Logout</a></li>');
	}
	$j(".search-toggler").click(function(e){
		e.preventDefault();
		$j("#header-search").slideToggle();
	});
	if($j(".product-view .old-price .price").length > 0 && $j(".product-view .special-price .price").length > 0){
		var old_price = $j(".product-view .old-price .price").html();
		old_price = old_price.replace(/\$/,'');
		console.log(old_price);
		var special_price = $j(".product-view .special-price .price").html();
		special_price = special_price.replace(/\$/,'');
		console.log(special_price);

		if(! isNaN(old_price) && ! isNaN(special_price)){
			var p = $j(".product-view .special-price .price");
			var delta = 100-Math.round((special_price/old_price)*100);
			p.before('<em>NOW</em>');
			p.after('<em>'+delta+'% OFF</em>');
		}
	}
...
*/
	...
	if($j(window).width() <=770){
		//$j(".vh-catalog-ads").css({"margin-top":$j(".shop-menu").height()+"px"})
	}
	if($j(window).width() <=479){
		$j(".vh-catalog-ads").css({"opacity":"0"})
	}
	if($j(window).width() <=770){
		$j(".level0.parent > a").attr("href","#");
		$j(".level0.parent > a").click(function(e){
			e.preventDefault();
		});
	}
	var menu_col_ctr = 0;
	var menu_row_ctr = 1;
	//var total_menu = $j(".category-menu-inner > ul > li.level0").length;
	...
	$j(window).on('resize', function(e)
	{
		...
		if($j(".product-detail-section").length > 0 || $j("body").hasClass("cms-about") || $j("body").hasClass("cms-privacy") || $j("body").hasClass("cms-terms") || $j("body").hasClass("cms-shipping") || $j("body").hasClass("cms-returns") || $j("body").hasClass("cms-howtovote") || $j("body").hasClass("cms-contact")){
